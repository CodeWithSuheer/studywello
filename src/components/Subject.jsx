        ...
        <>
            <section className='bg-white'>
                <div className="max-w-7xl mx-auto py-20 px-6 lg:px-0">
                    ...
                    <div className="mt-10 flex justify-center items-center flex-wrap gap-x-5 gap-y-5">
                        {subjectName?.map((data, index) => (
                            <div key={index} className='h-10 flex items-center'>
                                <button className={`text-md font-medium border px-6 py-2.5 rounded-xl text-white ${backgroundColors[index % backgroundColors.length]}`}>
                                    {data}
                                </button>
                            </div>
                        ))}
